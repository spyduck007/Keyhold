"""Binary vault container containing metadata and encrypted blobs."""
...
class VaultContainer:
    """The complete serialized state of one encrypted vault."""
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(blob, EncryptedBlob) for blob in self.blobs):
    ...
    def to_bytes(self) -> bytes:
        """Serialize the current version 2 container format."""
    ...
    def from_bytes(
        ...
        """Parse current and legacy vault containers."""
        try:
    ...
    def _from_current_bytes(
        ...
        for _ in range(blob_count):
            ...
            if (
                ...
                or ciphertext_length > MAX_BLOB_CIPHERTEXT_LENGTH
            ):
...
def _validate_lengths(
    *,
    ...
    if (
        manifest_ciphertext_length < AES_GCM_TAG_LENGTH
        or manifest_ciphertext_length > MAX_ENCRYPTED_MANIFEST_LENGTH
    ):
